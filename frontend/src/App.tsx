...
function App() {
  ...
  const svS=()=>{fetch("/api/config",{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({ollama_url:cu,model:cm})}).then(()=>ssh(false));};
  ...
  return (
    <div className="flex h-screen" style={{backgroundColor:S.c}}>
      <aside className="w-56 flex flex-col gap-0.5 p-4 border-r overflow-y-auto" style={{borderColor:S.h}}>
        ...
        <div className="mt-auto pt-4" style={{borderTop:`1px solid ${S.h}`}}>
          <button onClick={()=>{ssh(!sh);if(!sh){fetch("/api/config").then(r=>r.json()).then(c=>{scu(c.ollama_url);scm(c.model);});}}} className="w-full text-left px-3 py-1.5 rounded-md text-sm" style={{fontFamily:"'Inter',sans-serif",backgroundColor:sh?S.d:"transparent",color:sh?S.i:S.m}}>⚙ 设置</button>
        </div>
      </aside>
      ...
      <main className="flex-1 overflow-y-auto p-6">
        ...
        {vw==="grid"?(
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
            {fs.map(a=>(<div key={a.id} onClick={()=>selA(a.id)} className="rounded-xl overflow-hidden cursor-pointer transition-shadow hover:shadow-sm" style={{backgroundColor:S.c,border:sel?.id===a.id?`2px solid ${S.r}`:`1px solid ${S.h}`}}>
              <div className="aspect-square flex items-center justify-center relative" style={{backgroundColor:S.s}}>
                {a.thumbnail_status==="done"?<img src={`/api/thumbnails/${a.id}`} className="w-full h-full object-cover"/>:a.asset_type==="image"?<span className="text-sm animate-pulse" style={{color:S.m}}>生成中...</span>:<span className="text-4xl">{a.asset_type==="video"?"🎬":a.asset_type==="audio"?"🎵":"📄"}</span>}
              </div>
              <div className="p-3"><p className="text-sm font-medium truncate" style={{fontFamily:"'Inter',sans-serif",color:S.i}}>{a.filename}</p><p className="text-xs mt-0.5" style={{color:S.ms}}>{a.width&&a.height?`${a.width}×${a.height}`:a.duration?`${Math.round(a.duration)}秒`:""} {f(a.size)}</p>
      ...
      {sel&&<aside className="w-80 border-l overflow-y-auto p-4 flex flex-col gap-3" style={{borderColor:S.h,backgroundColor:S.c}}>
        <div className="flex justify-between items-center"><h2 className="text-sm font-medium truncate flex-1" style={{fontFamily:"'Inter',sans-serif",color:S.i}}>{sel.filename}</h2><button onClick={()=>sl(null)} className="text-lg leading-none px-1" style={{color:S.ms}}>✕</button></div>
        {sel.thumbnail_status==="done"?<img src={`/api/thumbnails/${sel.id}`} className="w-full rounded-lg" style={{border:`1px solid ${S.h}`}}/>:<div className="w-full aspect-square rounded-lg flex items-center justify-center" style={{backgroundColor:S.s}}><span className="text-5xl">{sel.asset_type==="video"?"🎬":sel.asset_type==="audio"?"🎵":"📄"}</span></div>}
        <div><div className="text-[11px] mb-0.5" style={{color:S.ms}}>路径</div><p className="text-xs break-all" style={{color:S.b}}>{sel.path}</p></div>
